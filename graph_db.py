...
# .env 의 NEO4J_DATABASE_GRAPH 로 'ai_arxiv_graph' 같은 이름을 지정한다.
GRAPH_DB_LABEL = "ai_arxiv_graph"   # 로그/리포트용 식별자 (실제 DB 이름과는 별개)
GRAPH_DB_NAME = os.getenv("NEO4J_DATABASE_GRAPH", "8448f3ad")
...
def _ensure_database(name: str) -> None:
    ...
    try:
        ...
        print(
            ...
            f"        NEO4J_DATABASE_GRAPH=neo4j 로 두세요. (기본 DB 사용)\n"
...
def _has_embedded_entities(name: str) -> bool:
    """Hybrid 빌드 잔존물 감지용 — :__Entity__ 노드에 embedding 이 붙어있으면 True."""
    driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USERNAME, NEO4J_PASSWORD))
    try:
        with driver.session(database=name) as ses:
            row = ses.run(
...
# _ensure_database(GRAPH_DB_NAME)

# Aura/Community 처럼 Graph 와 Hybrid 가 같은 'neo4j' DB 를 공유하는 경우
# 잘못된 flavor 잔존물을 빨리 잡아낸다.
if _db_has_data(GRAPH_DB_NAME) and _has_embedded_entities(GRAPH_DB_NAME):
    print(
        f"[error] DB '{GRAPH_DB_NAME}' 에 노드 임베딩이 붙은 그래프(Hybrid 잔존물)가 있어\n"
        f"        Graph RAG 로 로드할 수 없습니다. 먼저 reset 후 다시 실행하세요:\n"
        f"            python scripts/reset_neo4j.py --flavor graph"
    )
    raise SystemExit(1)


# Graph DB
if not _db_has_data(GRAPH_DB_NAME):
    print("Graph DB 구축 시작...")
    ...
    index = PropertyGraphIndex(
        ...
        embed_kg_nodes=False,        # ★ Graph RAG: (Ns,R,Nt) 트리플만, 노드 임베딩 없음
        show_progress=True,
    )
